// build-geo-posts-onboardplan.mjs
// Reads GEO article markdown files from opc-doc (onboardplan), extracts frontmatter + JSON-LD +
...
const ARTICLES_DIR = 'E:/AgentCPM/07_一人公司出海项目/opc-doc/outputs/07-conversion/articles-onboardplan'
const OUT_TS = 'E:/AgentCPM/07_一人公司出海项目/12_Micro_SaaS出海/onboardplan/data/geoPosts.ts'
// NOTE: Vercel alias is NOT onboardplan.lxsaihub.com — after first deploy, replace both with the
// real alias (e.g. onboardplan-<random>.vercel.app) and re-run this script + redeploy.
const PROD_URL = 'https://onboardplan.lxsaihub.com'
const LIVE_DOMAIN = 'https://onboardplan.lxsaihub.com'
...
function parseCta(rest) {
  ...
  if (!line) return ''
  const text = line.replace(/\[WAITLIST_URL\]/g, `<a href="${PROD_URL}" class="underline font-semibold">try OnboardPlan</a>`)
  return '<p>' + inline(text) + '</p>'
}

function fixDomain(obj) {
  return JSON.parse(JSON.stringify(obj).replace(/https:\/\/onboardplan\.vercel\.app/g, LIVE_DOMAIN))
}
...
for (const f of files) {
  ...
  const description = (blogPosting.description || '').replace(/https:\/\/onboardplan\.vercel\.app/g, LIVE_DOMAIN)
  posts.push({
    slug: fm.slug,
    title: fm.title || blogPosting.headline || f,
    datePublished: fm.date || blogPosting.datePublished || '2026-07-27',
    dateModified: blogPosting.dateModified || fm.date || '2026-07-27',
    description,
    html: mdToHtml(body).replace(/https:\/\/onboardplan\.vercel\.app/g, LIVE_DOMAIN),
    faq: parseFaq(faq),
...
const ts = `// AUTO-GENERATED by scripts/build-geo-posts-onboardplan.mjs — do not edit by hand.
